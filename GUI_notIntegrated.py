...
class GraphPage(QtWidgets.QWidget):
    ...
    def load_excel(self, file_path, is_celsius=True):
        ...
        try:
            ...
            if not is_celsius:
                y_data = (df_filtered['Temperature'] * 9/5) + 32
                y_label = "Degrees (°F)"
                y_min, y_max = 60.8, 86
            ...
            self.ax.clear()
            self.apply_style()
            self.ax.text(0.5, 0.5, f"Excel Error:\n{str(e)}", 
                         color='red', ha='center', va='center', transform=self.ax.transAxes)
            self.canvas.draw()
...
class DashboardWidget(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.lbl_ctrl.setGeometry(30, 110, 150, 20); self.lbl_ctrl.setStyleSheet(label_style)
        ...
        self.lbl_sys.setGeometry(30, 230, 150, 20); self.lbl_sys.setStyleSheet(label_style)
        self.btn_sys = self.make_styled_button("HEAT", 30, 260, btn_w, active=False)

        self.lbl_fan = QtWidgets.QLabel("FAN", self)
        self.lbl_fan.setGeometry(W - 190, 110, 160, 20); self.lbl_fan.setAlignment(QtCore.Qt.AlignRight); self.lbl_fan.setStyleSheet(label_style)
        ...
        self.plus_btn.setGeometry(int(W/2) + 45, 375, 85, 42); self.plus_btn.clicked.connect(self.p.inc_temp)
        self.plus_btn.setStyleSheet(pill_style); self.plus_btn.setCursor(QtCore.Qt.PointingHandCursor)
